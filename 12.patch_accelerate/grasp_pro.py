...
class Grasps:
    ...
    def load_from_jacquard_files(cls,jacquard_grasp_files,scale = 1.0):
        ...
        with open(jacquard_grasp_files) as f:
            for line in f:
                ...
                grasp_rectangles.append(Grasp_cpaw(np.array([y,x]),-theta/180.0*np.pi,h,w).as_gr)#我这边读取的顺序跟GGCNN中的有些不同,now.they are totally same.
    ...
    def generate_grid_mask(self,shape = (300,300)):
        pos_out = np.zeros(shape)
        ...
        try:
            for gr in self.grs:
                gt_centers.append(gr.center)
                gt_points.append(gr.points)
                gt_angles.append(gr.angle)
                gt_widths.append(gr.width)
                gt_lengths.append(gr.length)
                if max(gr.center+offset) > 300 or min(gr.center-offset) < 0:
                    continue
                x = np.arange(gr.center[0]-offset,gr.center[0]+offset,2)
                y = np.arange(gr.center[1]-offset,gr.center[1]+offset,2)

                xx,yy = np.meshgrid(x,y)
                if len(rr) != 0:
                    rr = np.concatenate((rr,xx),axis = 1)
                    cc = np.concatenate((cc,yy),axis = 1)
                else:
                    rr = xx
                    cc = yy
            pos_out[rr,cc] = 1.0
            ...
            print(e)
            print(1)
        gt_num = len(gt_angles)
        while len(gt_angles) < 1200:
            gt_centers.append(gr.center)
            gt_points.append(gr.points)
            gt_angles.append(gr.angle)
            gt_widths.append(gr.width)
        ...
        return pos_out, gt_num, np.stack(gt_points),np.stack(gt_centers), np.stack(gt_angles).astype(np.float32), np.stack(gt_widths).astype(np.float32), np.stack(gt_lengths).astype(np.float32)

    def generate_img_n(self,pos = True,angle = True,width = True,shape = (300,300)):
        '''
        :功能       :将本对象的多个的抓取框信息融合并生成指定的映射图，以这种方式返回定义一个抓取的多个参数，包括中心点，角度，宽度,区别在与,这个是会根据抓取框距离中心的距离来给中间值赋亮度
        :参数 pos   :bool,是否生成返回位置映射图
        :参数 angle :bool,是否生成返回角度映射图
        :参数 width :bool,是否生成返回夹爪宽度映射图
        :参数 shape :tuple
