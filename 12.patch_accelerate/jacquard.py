...
class Jacquard(torch.utils.data.Dataset):
    #载入Jacquard数据集的类
    def __init__(self,file_dir,include_depth = True,include_rgb = True,start = 0.0,end = 1.0,ds_rotate = 0,random_rotate = False,random_zoom = False,output_size = 300,load_from_npy = False,npy_path = None):
        '''
        参数
        ----------
        file_dir : str
        ...
        if load_from_npy:
            graspf = np.load(os.path.join(file_dir,npy_path)).tolist()
        graspf.sort()
        ...
        mask_prob = [filename.replace('grasps.txt','prob_15.png') for filename in graspf]
    ...
    def __getitem__(self,idx):
        ...
        # 载入抓取标注参数
        pos_img,angle_img,width_img,poto_data = self.get_grasp(idx,rot = rot,zoom = zoom_factor)
        ...
        rgb = self.get_rgb(idx,rot,zoom_factor,normalize=False)
        mask_d_1 = self.get_mask_d_1(idx,rot,zoom_factor)
        mask_prob = self.get_mask_prob(idx,rot,zoom_factor)
        return x,(pos_img,cos_img,sin_img,width_img),idx,rot,zoom_factor,(poto_data,mask,rgb,mask_d_1,mask_prob,idx) #这里多返回idx,rot和zomm_facor参数，方便后面索引查找真实标注，并对真实的标注做同样处理以保证验证的准确性
    #映射类型的数据集，别忘了定义这个函数
    def __len__(self):
