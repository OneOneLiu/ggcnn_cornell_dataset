...
class Jacquard(torch.utils.data.Dataset):
    ...
    def __getitem__(self,idx):
        ...
        mask_prob = self.numpy_to_torch(mask_prob)

        mask_d_1 = self.get_mask_d_1(idx,rot,zoom_factor)
        mask_d_1 = self.numpy_to_torch(mask_d_1)

        return x,(pos_img,cos_img,sin_img,width_img,mask_prob,mask_d_1),idx,rot,zoom_factor #这里多返回idx,rot和zomm_facor参数，方便后面索引查找真实标注，并对真实的标注做同样处理以保证验证的准确性
    #映射类型的数据集，别忘了定义这个函数
    def __len__(self):
