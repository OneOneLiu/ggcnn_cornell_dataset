...
import numpy as np
from functions import show_grasp, get_edge
...
class GGCNN2(nn.Module):
    ...
    def compute_loss(self, xc, yc):
        y_pos, y_cos, y_sin, y_width, y_prob, y_height= yc
        ...
        if flag:
            prob_s = torch.cat((y_pos,y_prob,pos_pred,prob),dim = 0)
            for i in range(32):
                plt.subplot(4,8,i+1)
                plt.imshow(prob_s[i][0].cpu().data.numpy())
            plt.show()
        
        prediction = (prob,cos_pred,sin_pred,width_pred)
        gt_patches,pre_patches = self.get_patch_no_dis(prediction,y_height)
        if type(gt_patches) == 'int':
            return 0
        patch_loss = F.mse_loss(pre_patches, gt_patches)

        return {
            'loss': 0.5*cos_loss + 0.5*sin_loss + 0.5*width_loss + prob_loss + patch_loss,
            'losses': {
                'p_loss': p_loss,
                'cos_loss': cos_loss,
                'sin_loss': sin_loss,
    ...
    def get_patch_no_dis(self,prediction,mask_height):
        ...
        for i in range(batch_size):
            ...
            if pre_patch.shape != gt_patch.shape:
                print(1)
...
def get_patch_params(i,old_y,old_x,selected_edge,width_patch,angle,directions,left_right_diff,top_bottom_diff):
    # 为其生成对应的优化参数
    scale = 1
    if selected_edge * width_patch * 1.50 < 2:
        scale = 1.3
    ...
    if directions[i] == 1: # 向右移
        ...
        step = top_bottom_diff * width_patch *1.50  # 移动宽度的0.5倍
