...
from functions import post_process, detect_grasps, max_iou, collision_validate, show_grasp
net_path = '/home/ldh/github_repositorys/ggcnn_cornell_dataset/12.patch_accelerate/trained_models/filter'
model_path = '210910_2241/model0.927_epoch25_batch_8.pth'
# 一些训练参数的设定
batch_size = 8
epochs = 100
c_epochs = 40
            ...
            for x, y, idx, rot, zoom_factor in val_data:
                ...
                if len(grasp_pres) > 0:
                    ...
                    if edge > 5:
                        ...
                        # img_name = '12.patch_accelerate/output/correct/c_{0}_{1}_{2}.png'.format(idx.cpu().data.numpy()[0],rot.cpu().data.numpy()[0],zoom_factor.cpu().data.numpy()[0])
                        # gr = grasp_pre.as_gr
                        ...
                        # rgb = val_data.dataset.get_rgb(idx, rot, zoom_factor, normalize = False)
                        # img_name = '12.patch_accelerate/output/failed/f_{0}_{1}_{2}.png'.format(idx.cpu().data.numpy()[0],rot.cpu().data.numpy()[0],zoom_factor.cpu().data.numpy()[0])
                        # gr = grasp_pre.as_gr
                        # img = show_grasp(rgb,gr,255)
                        # imsave(img_name, img)
                else:
...
def run():
    ...
    net.load_state_dict(torch.load(os.path.join(net_path,model_path)))
    net = net.to(device)
    # 保存网络和训练参数信息
