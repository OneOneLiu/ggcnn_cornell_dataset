...
class Image:
    ...
    def zoom(self, factor, mask = None):
        """
        ...
        if factor < 1.0:
            ...
            scale = 1/factor
            if abs(sum(self.img[0] - self.img[-1])) < 1:
                # print('横向渐变')
                is_horizontal = True
            else:
                # print('纵向渐变')
                is_horizontal = False
            origin_backgroud = get_gradation_2d(self.img[0][0],self.img[-1][-1],1024,1024,is_horizontal=is_horizontal)
            mask_depth = mask * (self.img-origin_backgroud)
            backgroud = get_gradation_2d(self.img[0][0] / scale,self.img[-1][-1] / scale,1024,1024,is_horizontal=is_horizontal)
            # 缩小图片
            scaled_img = rescale(mask_depth,[scale,scale])
            # 填充图片
            paded = np.pad(scaled_img,int((1024*(1-scale))/2),pad_width)
            # 缩放图片
            paded_img = resize(paded, (1024,1024), preserve_range=True).astype(paded.dtype)
