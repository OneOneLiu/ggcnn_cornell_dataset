...
from .grasp_data_scale import GraspDatasetBase
...
class JacquardDataset(GraspDatasetBase):
    ...
    def get_imgs(self, idx, rot=0, zoom=1.0):
        ...
        if len(grasps2.grs) > 1:
            ...
            # self.output_size))
            # rgb_img = self.get_rgb(idx, rot, zoom = 1.0, normalise = False)
            # plt.subplot(4,4,1)
            # plt.title('origin pos')
            # plt.imshow(pos_img_o)
            # plt.subplot(4,4,2)
            # plt.title('origin ang')
            # plt.imshow(ang_img_o)
            # plt.subplot(4,4,3)
            # plt.title('origin width')
            # plt.imshow(width_img_o)
            # plt.subplot(4,4,4)
            # plt.title('mask')
            # plt.imshow(mask_img)
            # plt.subplot(4,4,5)
            # plt.title('typical pos')
            # plt.imshow(pos_img)
            # plt.subplot(4,4,6)
            # plt.title('typical ang')
            # plt.imshow(ang_img)
            # plt.subplot(4,4,7)
            # plt.title('typical width')
    ...
    def get_depth(self, idx, rot=0, zoom=1.0):
        ...
        depth_img.normalise()
        # cache = copy.copy(depth_img.img)
        mask = None
        if zoom  > 1.0:
            mask_img = image.DepthImage.from_tiff(self.maskf[idx])
            mask_img.rotate(rot)
            mask = (mask_img.img//50.0).astype(np.bool)
        depth_img.zoom(zoom,mask)
    ...
    def get_scale(self,idx):
        ...
        self.scale = np.array([size_percentage/0.2,x_range/200,y_range/200,0.3]).max()
