...
class GraspRectangles:
    ...
    def generate_pos_img(self,shape = (300,300)):
        pos_out = np.zeros(shape)
        for gr in self.grs:
            rr,cc = gr.compact_polygon_coords_n(shape)#shape的指定还是很重要的，可以考虑图像边界
            pos_out[rr,cc] = 1
    ...
    def cal_pos_intensity(self,grs_center,gr,max_d):
        ...
        if max_d == 0:
            # print(max_d)
            return 1
        intensity = 1-round(distance/max_d,3)/2.0
...
class GraspRectangle:
    ...
    def compact_polygon_coords_n(self, shape=None):
        """
        :param shape: Output shape
        :return: Indices of pixels within the centre thrid of the grasp rectangle.
        """
