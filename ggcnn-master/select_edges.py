...
from utils.dataset_processing.evaluation import collision_validate,show_grasp
...
save_folder = os.path.join('output/pictures/', net_desc)
...
model_path = 'output/models/211128_1606_/epoch_49_iou_0.85_statedict.pt'
    ...
    with torch.no_grad():
        ...
        while batch_idx < batches_per_epoch:
            for x, y, didx, rot, zoom_factor in val_data:
                ...
                # 真实精度检测
                if len(gs) > 0:
                    gr_img = show_grasp(yc[5].squeeze().cpu().data.numpy(),gs[0].as_gr,int(yc[5].max().cpu().data.numpy()))
                    ...
                    for edge_threshold in range(0,20):
                        if edge > edge_threshold:
                            ...
                            img_name = 'output/pictures/{0}/correct/{1}_{2}_{3}.png'.format(edge_threshold,didx.cpu().data.numpy()[0],rot.cpu().data.numpy()[0],zoom_factor.cpu().data.numpy()[0])
                        else:
                            results['negative_'+str(edge_threshold)] += 1
                            img_name = 'output/pictures/{0}/fail/{1}_{2}_{3}.png'.format(edge_threshold,didx.cpu().data.numpy()[0],rot.cpu().data.numpy()[0],zoom_factor.cpu().data.numpy()[0])
                        imsave(img_name, gr_img)
                    if edge > 4 and lr_congruous < 0.85 and tb_congruous < 0.85:
                        results['perfect'] += 1
                        img_name = 'output/pictures/perfect/{1}_{2}_{3}.png'.format(edge_threshold,didx.cpu().data.numpy()[0],rot.cpu().data.numpy()[0],zoom_factor.cpu().data.numpy()[0])
                    else:
                        img_name = 'output/pictures/not_perfect/{1}_{2}_{3}.png'.format(edge_threshold,didx.cpu().data.numpy()[0],rot.cpu().data.numpy()[0],zoom_factor.cpu().data.numpy()[0])
                        results['not_perfect'] += 1
                        if s == 1:
                            img_name = 'output/pictures/false_positive/{1}_{2}_{3}.png'.format(edge_threshold,didx.cpu().data.numpy()[0],rot.cpu().data.numpy()[0],zoom_factor.cpu().data.numpy()[0])
                    imsave(img_name, gr_img)
                else:
                    for edge_threshold in range(0,20):
                        img_name = 'output/pictures/{0}/fail/{1}_{2}_{3}.png'.format(edge_threshold,didx.cpu().data.numpy()[0],rot.cpu().data.numpy()[0],zoom_factor.cpu().data.numpy()[0])
...
if __name__ == '__main__':
    # 这个程序就是把各个不同edge的分类识别结果保存到本地,来选取一个合适的edge,仅validate
    for i in range(30):
        run()