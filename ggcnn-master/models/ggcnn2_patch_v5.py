...
class GGCNN2(nn.Module):
    ...
    def forward(self, x):
        ...
        filter_pos = self.filter_pos(x)
        filter_cos = self.filter_cos(x)
        filter_sin = self.filter_sin(x)
        filter_width = self.filter_width(x)

        return pos_output, cos_output, sin_output, width_output, filter_pos, filter_cos, filter_sin, filter_width

    def compute_loss(self, xc, yc):
        y_pos, y_cos, y_sin, y_width, mask_prob, y_height = yc
        pos_pred, cos_pred, sin_pred, width_pred, filter_pos, filter_cos, filter_sin, filter_width = self(xc)
        ...
        filtered_pos = filter_pos.sigmoid() * pos_pred
        ...
        pos_loss1 = F.mse_loss(filtered_pos * y_pos, y_pos * y_pos)
        ...
        prediction = (filtered_pos,filtered_cos,filtered_sin,filtered_width)
        gt_patches, pre_patches = self.get_patch_no_dis(prediction, y_height)

        patch_loss = F.mse_loss(pre_patches, gt_patches)

        return {
            'loss': p_loss + cos_loss + sin_loss + width_loss + pos_loss1 + cos_loss1 + sin_loss1 + width_loss1 + patch_loss,
            'losses': {
                'p_loss': p_loss,
                'cos_loss': cos_loss,
                'sin_loss': sin_loss,
