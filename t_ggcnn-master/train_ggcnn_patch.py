...
save_folder = os.path.join('output/3.patch/', net_desc)
...
model_path = './output/3.patch/220426_1032_/00_epoch_81_iou_0.74_statedict.pt'
...
def parse_args():
    ...
    # Network
    parser.add_argument('--network', type=str, default='ggcnn2_patch', help='Network Name in .models')

    # Dataset & Data & Training
    parser.add_argument('--dataset', type=str, default = 'jacquard', help='Dataset Name ("copatrnell" or "jaquard")')
...
def train(epoch, net, device, train_data, optimizer, batches_per_epoch):
    ...
    while batch_idx < batches_per_epoch:
        for x, y, _, _, _ in train_data:
            ...
            yc = [yy.to(device) for yy in y]
            include_patch = 0
            if epoch > patch_epoch:
                include_patch = 1
            lossd = net.compute_loss(xc, yc, include_patch)
            # lossd = net.compute_loss(xc, yc, 1)
            loss = lossd['loss']
...
def print_result(val_results):
    ...
    name_list = ['iou_acc','iou_prob_acc','iou_patch_acc']
    perfect_name_list = ['perfect_acc','perfect_prob_acc','perfect_patch_acc']
    edge_name_list = ['','prob','patch_']
    for pos, name, edge_name, perfect_name in zip(map_list,name_list,edge_name_list,perfect_name_list):
        ...
        for i in range(edge_num):
            logger.info('edge_' + edge_name + '%d:%d/%d = %f' % (i,test_results['positive_'+str(i)], test_results['positive_'+str(i)] + test_results['negative_'+str(i)],test_results['positive_'+str(i)]/(test_results['positive_'+str(i)]+test_results['negative_'+str(i)])))
