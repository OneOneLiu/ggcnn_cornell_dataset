...
class Jacquard(torch.utils.data.Dataset):
    ...
    def __getitem__(self,idx):
        ...
        edge_img = self.get_edges(idx,rot,zoom_factor)
        edge_img = self.numpy_to_torch(edge_img)
        
        return x,(pos_img,cos_img,sin_img,width_img),idx,rot,zoom_factor,edge_img#这里多返回idx,rot和zomm_facor参数，方便后面索引查找真实标注，并对真实的标注做同样处理以保证验证的准确性
    #映射类型的数据集，别忘了定义这个函数
    def __len__(self):
