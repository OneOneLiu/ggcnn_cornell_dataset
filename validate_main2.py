...
这个程序就将验证部分嵌入到整个训练过程中去了，既有训练部分又有验证部分，在每个epoch的结尾验证性能，同时需要注意划分验证集和训练集了
